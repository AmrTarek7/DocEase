...
export class AppConfigurator {
  ...
  ngOnInit() {
    ...
    if (isPlatformBrowser(this.platformId)) {
      const storedLayoutConfig = localStorage.getItem('layoutConfig');
      if (storedLayoutConfig) {
        const layoutConfig = JSON.parse(storedLayoutConfig);
  ...
  updateColors(event: any, type: string, color: any) {
    ...
    if (type === 'primary') {
      updatedState = { primary: color.name };
    } else if (type === 'surface') {
      updatedState = { surface: color.name };
    }

    // تحديث الـ layoutConfig
    this.layoutService.layoutConfig.update((state) => ({
      ...state,
      ...updatedState,
    }));
    ...
    localStorage.setItem(
      'layoutConfig',
      JSON.stringify(this.layoutService.layoutConfig())
    );
  ...
  onPresetChange(event: any) {
    ...
    // حفظ الـ preset في localStorage
    localStorage.setItem(
      'layoutConfig',
      JSON.stringify(this.layoutService.layoutConfig())
    );
  ...
  onMenuModeChange(event: string) {
    ...
    // حفظ الـ menuMode في localStorage
    localStorage.setItem(
      'layoutConfig',
      JSON.stringify(this.layoutService.layoutConfig())
    );
